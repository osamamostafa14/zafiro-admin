...
class OrderController extends Controller
{
    ...
    {
        return view('branch-views.order.index');
    }
    
    public function add_new_route($order_id)
    {
        $order = Order::find($order_id);
        return view('branch-views.order.routes.index', compact('order'));
    }
    
    public function add_route($order_id)
    {
        $order = Order::find($order_id);
        return view('branch-views.order.routes.add-route', compact('order'));
    }
    ...
    public function edit($order_id)
    {
        ...
        return view('branch-views.order.edit', compact('order',  'date', 'time'));
    }

    public function updateOrderStatus(Request $request, $id)
    {
        $request->validate([
            'new_status' => 'required|in:canceled,pending,delivered,processing,undelivered',
        ]);
    ...
    public function edit_routes($order_id)
    {
      ...
      //  $takendrivers = OrderRoute::with('delivery_routes')->where('order_id', $order->id)->distinct()->get('driver_id');
        ...
        return view('branch-views.order.routes.edit', 
    ...
    {        
       ...
       // return redirect()->route('branch.order.routes.edit', ['order_id' => $order->id]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $order->branch_id =  Auth::guard('branch')->user()->id; 

        $order->date = date('Y-m-d H:i:s', $timestamp);
        $order->save();
        
        Toastr::success('Order created!');
        return redirect()->route('branch.order.list');
    }
    ...
    {
       if($order_status == 'all'){
           $orders = Order::where(['branch_id' => Auth::guard('branch')->user()->id])->paginate(10);
         
       }else{
         $orders = Order::where(['order_status' =>  $order_status , 'branch_id' => Auth::guard('branch')->user()->id])->paginate(10);
       }
       
        return view('branch-views.order.list', compact('orders'));
    }
    ...
    {
        $order = Order::where(['id' => $request->order_id , 'branch_id' => Auth::guard('branch')->user()->id ]);
    ...
    {
        // $order = Order::find($request->order_id);
        $order = Order::where(['id' => $request->order_id , 'branch_id' => Auth::guard('branch')->user()->id ])->first();
    ...
    public function assign_driver($order_id)
    {
        ...
        $order =  Order::where(['id' => $order_id , 'branch_id' => Auth::guard('branch')->user()->id ])->first();
        
        // $order = Order::find($order_id);
      
        return view('branch-views.order.driver.assign-driver', compact('drivers','order')); 
    }
    ...
    public function driver_details($driver_id,$order_id){
        ...
        $order =  Order::with('order_routes')->where(['id' => $order_id , 'branch_id' => Auth::guard('branch')->user()->id ])->first();
        // $order = Order::with('order_routes')->find($order_id);        
        ...
        return view('branch-views.order.driver.driver-details', compact('driver_id', 'order', 'waypoints_arr', 'selected_route_id', 'order_routes', 'saved_total_duration', 'saved_total_distance','driverroutes','order_id'));
    ...
    public function store_assign(Request $request)
    {
        ...
        $order =  Order::with('order_routes')->where(['id' =>$request->order_id , 'branch_id' => Auth::guard('branch')->user()->id ])->first();
