            ...
            <div class="row align-items-center mb-3">
                <div class="col-sm">
                    <h1 class="page-header-title">Orders
                        ...
                        <span class="badge badge-soft-dark ml-2">{{\App\Model\Order::where(['branch_id' =>  Auth::guard('branch')->user()->id])->count()}}</span>
                    </h1>
                </div>
            </div>
            <!-- End Row -->
                ...
                <div class="row justify-content-between align-items-center flex-grow-1">
                    ...
                    <div class="col-lg-3">
                        <select class="form-control filter-status" name="filter-status">
                            <option value="{{ route('branch.order.list-search',['all']) }}" >Show All</option>
                            @foreach($OrderStatusArr as $status)
                            <option 
                            @if($status == request()->route('order_status'))
                            selected
                            @endif
                            value="{{ route('branch.order.list-search',[$status]) }}">{{ ucwords($status) }}</option>
                            @endforeach 
                        </select>
                    </div>
                </div>
                    ...
                    @php($routes_count = count($order->order_routes))
                        <tr class="">
                            ...
                            <td> 
                                ...
                                <form action="{{ route('branch.order.update.status', ['id' => $order->id]) }}" id="form{{ $order->id }}" method="post">
                                    @csrf
                                    @method('PUT') 
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <a class="dropdown-item" href="{{route('branch.order.edit',[$order['id']])}}">
                                            <i class="tio-visible"></i> {{trans('messages.view')}}
                                        </a>
                                        
                                        <a class="dropdown-item" href="{{route('branch.order.routes.add-route',[$order['id']])}}">
                                            <i class="tio-add-circle-outlined"></i> Add new route
                                        </a>
                                        
                                        <a class="dropdown-item" href="{{route('branch.order.routes.edit',[$order['id']])}}">
                                            <i class="tio-edit"></i> Edit routes
                                        </a>
                                        
                                        <a class="dropdown-item" href="{{route('branch.order.driver.assign-driver',[$order['id']])}}">
